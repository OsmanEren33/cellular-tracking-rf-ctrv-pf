...
5) Random Forest girişi [x_m, y_m, sin_phi, cos_phi, distance], çıkışı 8 radyo parametresidir.
6) Yalnızca split tablo figürü, takip grafiği ve hata CDF grafiği gösterilir.

RF girişi  : [x_m, y_m, sin_phi, cos_phi, distance]
...
RF_INPUTS = ["x_m", "y_m", "sin_phi", "cos_phi", "distance"]
...
def load_and_prepare_data(excel_path):
    ...
    # Random Forest için baz istasyonuna göre geometrik konum özellikleri.
    # Referans alınan açı mantığı:
    #   phi = atan2(y_m, x_m)
    #   sin_phi = sin(phi)
    #   cos_phi = cos(phi)
    #   distance = sqrt(x_m^2 + y_m^2)
    # Buradaki phi, baz istasyonundan veri noktasına olan yön açısıdır;
    # anten azimuth sütunu değildir.
    phi = np.arctan2(
        data["y_m"].to_numpy(dtype=float),
        data["x_m"].to_numpy(dtype=float),
    )
    data["sin_phi"] = np.sin(phi)
    data["cos_phi"] = np.cos(phi)
    data["distance"] = np.sqrt(
        data["x_m"].to_numpy(dtype=float) ** 2
        + data["y_m"].to_numpy(dtype=float) ** 2
    )
...
def build_rf_measurement_grid(final_rf, xy_bounds, grid_step_m):
    ...
    # Eğitimde kullanılan beş RF girişini ızgara noktaları için de aynı
    # geometrik tanımla üret.
    grid_x = xx.ravel()
    grid_y = yy.ravel()
    grid_phi = np.arctan2(grid_y, grid_x)
    grid_distance = np.sqrt(grid_x ** 2 + grid_y ** 2)
...
# =============================================================================
# 7) SONUÇLAR VE YALNIZCA ÜÇ FİGÜR
# =============================================================================

def show_split_comparison_table(summary_df, selected_split):
    """Train/test split karşılaştırmasını tablo figürü olarak gösterir."""
    table_df = summary_df[
        [
            "Split",
            "Train Rows",
    ...
    ax.set_title(
        f"Random Forest Train/Test Split Comparison - Selected Split: {selected_split}",
        pad=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def main():
    ...
    show_split_comparison_table(summary_df, selected["summary"]["Split"])
